...
Generated output is overwritten on every run, so hand-edits are silently lost. [`hooks/deny-generated-file-edits.sh`](hooks/deny-generated-file-edits.sh) is a Claude Code `PreToolUse` hook that prevents this: it denies `Edit` / `Write` / `NotebookEdit` on any distill-generated file and tells the agent where the real source lives.
...
Converting an existing hand-written `CLAUDE.md` to distill: [`docs/adopting-distill.md`](docs/adopting-distill.md).
